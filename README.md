...
- ELK, EFK에서
    - 데이터 전송/필터링/전처리 등 역할 담당
        - L:Logstash    -> 라이브러리 활용, 비정형 데이터(text 로그파일) => 가공 => json 전송
...
Python 로그 생성기 ┤
                  └─ sensor_text.log (비정형 데이터)
                         │
                         ▼
                    Fluent Bit  <- 컨셉상 2가지 방향성으로 설계한 것임(실제는 1개만 수행)
                    /         \
                   /           \
          JSON 직결             TEXT
             │                  │
             ...
             │                  │
             ▼                  ▼
 factory-json-topic      factory-text-topic   <- 토픽 : 카프카에서 메시지를 구분하는 용도
             \                 /
              \               /
